...
public class CapiServerApplication extends CapiCallApplication{
  ...
  public void run( ){                                 // Runnable interface
    try{
//      int ctrlid=md.getInt(capiControllerID,1);
      int cipmask=md.getInt("capi.cipmask");

      int noofctrls=CapiEnumerator.getNoOfControllers();
      for(int cntrl=1;cntrl<=noofctrls;cntrl++){
        put(new ListenReq(appid,cntrl,cipmask));
      }
  ...
  public void close(){
    try{
//      int cntrl=md.getInt(capiControllerID,1);
      int noofctrls=CapiEnumerator.getNoOfControllers();
      for(int cntrl=1;cntrl<=noofctrls;cntrl++){
